...
static int nbc_neighbor_allgather_init(const void *sbuf, int scount, MPI_Datatype stype, void *rbuf,
  ...
  search.rtype = rtype;
  found = (NBC_Ineighbor_allgather_args *) hb_tree_search ((hb_tree *) libpnbc_osc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER],
                                                           &search);
  if (NULL == found) {
#endif
    ...
    if (NULL != args) {
      ...
      res = hb_tree_insert ((hb_tree *) libpnbc_osc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER], args, args, 0);
      if (0 == res) {
        OBJ_RETAIN(schedule);

        /* increase number of elements for A2A */
        if (++libpnbc_osc_module->NBC_Dict_size[NBC_NEIGHBOR_ALLGATHER] > NBC_SCHED_DICT_UPPER) {
          NBC_SchedCache_dictwipe ((hb_tree *) libpnbc_osc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER],
                                   &libpnbc_osc_module->NBC_Dict_size[NBC_NEIGHBOR_ALLGATHER]);
        }
      } else {
