  ...
  {                                                                     \
    ...
    for (i=0; i<num; i++) {                                             \
      ...
      case SEND:                                                        \
        ...
        printf("*buf: %lu, count: %i, type: %lu, dest: %i)\n", (unsigned long)sendargs.buf, \
               sendargs.count, (unsigned long)sendargs.datatype, sendargs.dest); \
        break;                                                          \
      case RECV:                                                        \
        printf("[%i]  RECV (offset %li) ", myrank, (long)p-(long)schedule); \
        NBC_GET_BYTES(p,recvargs);                                      \
        printf("*buf: %lu, count: %i, type: %lu, source: %i)\n", (unsigned long)recvargs.buf, \
               recvargs.count, (unsigned long)recvargs.datatype, recvargs.source);            \
        break;                                                          \
      case OP:                                                          \
      ...
      case COPY:                                                        \
        ...
        printf("*src: %lu, srccount: %i, srctype: %lu, *tgt: %lu, tgtcount: %i, tgttype: %lu)\n",\
               (unsigned long)copyargs.src, copyargs.srccount, (unsigned long)copyargs.srctype, \
               (unsigned long)copyargs.tgt, copyargs.tgtcount, (unsigned long)copyargs.tgttype); \
        break;                                                          \
      case UNPACK:                                                      \
        ...
        printf("*src: %lu, srccount: %i, srctype: %lu, *tgt: %lu\n",(unsigned long)unpackargs.inbuf,\
               unpackargs.count, (unsigned long)unpackargs.datatype, (unsigned long)unpackargs.outbuf); \
        break;                                                          \
      default:                                                          \
...
#define NBC_PRINT_SCHED(schedule)                                       \
  ...
  */
  ...
  static inline int NBC_Copy(const void *src, int srccount, MPI_Datatype srctype, void *tgt,
                             int tgtcount, MPI_Datatype tgttype, MPI_Comm comm) {
    int res;
