...
int ompi_osc_rdma_ifree(ompi_win_t *win, ompi_request_t ** request)
{
    int ret = OMPI_SUCCESS;
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    ...
    if (NULL != module->comm) {
        ...
        /* finish with a barrier */
        if (ompi_group_size(win->w_group) > 1) {
            (void) module->comm->c_coll->coll_ibarrier (module->comm, request,
                                                        module->comm->c_coll->coll_ibarrier_module);
        }
    }
...
int ompi_osc_rdma_complete_ifree(ompi_win_t *win)
{
    int ret = OMPI_SUCCESS;
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    ompi_osc_rdma_peer_t *peer;
